...
var parseBoolean, MAX_FLOAT, expectString, stackTraceWarning, logger3;
...
var init_createConfigValueProvider = __esm({
  "node_modules/@smithy/middleware-endpoint/dist-es/adaptors/createConfigValueProvider.js"() {
    "use strict";
    createConfigValueProvider = (configKey, canonicalEndpointParamKey, config3) => {
  ...
  "node_modules/@smithy/node-config-provider/dist-es/fromEnv.js"() {
    ...
    fromEnv = (envVarSelector, logger4) => async () => {
      try {
        ...
        throw new CredentialsProviderError(e.message || `Not found in ENV: ${getSelectorName(envVarSelector.toString())}`, { logger: logger4 });
      }
    };
  }
});
...
var logSecurityEvent = (event, details) => {
...
var logAuthenticationFailure = (ip, reason, userAgent) => {
...
var logSuspiciousActivity = (activity, details) => {
...
var logDataAccess = (userId, resource, action, ip) => {
...
var getApiRequests = () => morgan((tokens, req, res) => {
  const status = tokens.status(req, res);
  const method = tokens.method(req, res);
  const url = tokens.url(req, res);
  const responseTime = tokens["response-time"](req, res);
  const statusNum = parseInt(status || "0");
  const coloredStatus = statusNum >= 500 ? chalk.red.bold(status) : statusNum >= 400 ? chalk.yellow.bold(status) : statusNum >= 300 ? chalk.cyan(status) : statusNum >= 200 ? chalk.green.bold(status) : chalk.white(status);
  const methodColors = {
    "GET": chalk.blue,
    "POST": chalk.green,
    "PUT": chalk.yellow,
  ...
  const emoji = statusNum >= 500 ? "\u2757" : statusNum >= 400 ? "\u26A0\uFE0F" : statusNum >= 200 ? "\u2705" : "\u{1F514}";
  ...
  runServer = (server, path) => {
    ...
    connection.use((socket, next) => {
      try {
        ...
        const stringifiedAccount = this.encryption.decryptId(token, Token.apiAccessToken);
        if (!stringifiedAccount.status) return next(new Error("authentication error"));
        const account = JSON.parse(stringifiedAccount.data);
    ...
    connection.on("connection", (client) => {
      ...
      this.socketInfo.forEach((info) => {
        client.on(info.path, (data, callBack) => {
          if (!callBack) callBack = (demo) => demo;
          info.action(
            {
              message: data,
              auth: {
...
var NewSocketConnector = (encryption) => {
  return new SocketConnector(encryption);
};

// src/shared/adapter/libraries/telegram/index.ts
import { Telegraf } from "telegraf";
var TelegrafRouterConstructor = class {
  encryption;
  botInfo = [];
  constructor(encryption) {
    this.encryption = encryption;
  ...
  start = (action) => {
    this.botInfo.push({ command: "start", action });
  };
  route = (command, action) => {
    this.botInfo.push({ command, action });
  };
  extendsRoutes = (router2) => {
    this.botInfo = [...this.botInfo, ...router2.botInfo];
  };
  runBot = (token) => {
    const bot = new Telegraf(token);
    this.botInfo.forEach((info) => {
      if (!info.command || !info.action) return;
    ...
    bot.launch();
    console.log("\u{1F680} Telegram bot running...");
  };
};
var TelegrafRouter = (encryption) => {
  return new TelegrafRouterConstructor(encryption);
};

// src/features/bot/onboard/onboard.controller.ts
...
var TransferController = class {
  ...
  welcome = async ({ ctx }) => {
    const response = await this.transferService.welcome(ctx.from);
    if (!response.status) {
      await ctx.reply("\u274C An error occurred. Please try again later.");
      return;
    }
    await ctx.reply(response.data.title, {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("\u{1F4CA} Lay Complaints", "user_complaints")],
        [Markup.button.callback("\u2699\uFE0F Settings", "settings")]
      ])
    });
  };
  userComplaints = async ({ ctx }) => {
    await ctx.editMessageText(`\u{1F4CA} *User Complaints*`, {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("\u{1F4CA} Transactions", "transactions")],
        [Markup.button.callback("\u{1F4CA} Complaints", "complaints")],
        [Markup.button.callback("\u2B05\uFE0F Back", "back_home")]
      ])
    });
  };
  // getTransactionHistory = async ({ ctx }: IBotRequest<string>) => {
  //   const response = await this.transferService.getTransactionHistory(ctx.from!);
  //   await ctx.editMessageText(`📊 *Transaction History*`, {
  //     parse_mode: "Markdown",
  //     ...Markup.inlineKeyboard([
  //       [Markup.button.callback("⬅️ Back", "back_home")],
  //     ]),
  ...
  // };
  backHome = async ({ ctx }) => {
    await ctx.editMessageText("\u{1F3E0} *Main Menu*", {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("\u{1F4B8} Start Transfer", "start_transfer")],
        [Markup.button.callback("\u{1F4E5} Receive Payment", "receive_payment")],
        [Markup.button.callback("\u2699\uFE0F Settings", "settings")]
      ])
    });
  };
};
var onboard_controller_default = TransferController;

// src/features/bot/onboard/onboard.service.ts
var OnboardService = class {
  ...
  async welcome(user) {
    return {
      status: true,
      data: {
        title: `\u{1F44B} Welcome ${user.first_name}!`
      },
      message: "Welcome to the Onboard Bot Service"
    };
  }
  // async getTransactionHistory(user: { id: number; first_name: string }, info: { id: string, email: string }): Promise<ResponseProps<IGeneralTransaction>> {
  //   const transaction = await this.model.transactions.findOne({
  //     where: { user: { email: info.email } },
  //   });
  //   return { status: true, data: transaction?.toGeneralResponse };
  // }
};
var onboard_service_default = OnboardService;

// src/features/bot/onboard/onboard.route.ts
var transferRouter = (data) => {
  const router2 = TelegrafRouter(data.repo.encryption);
  const controller = new onboard_controller_default(new onboard_service_default(data.model, data.repo));
  router2.start(controller.welcome);
  return router2;
};
var onboard_route_default = transferRouter;

// src/features/bot/bot.route.ts
var botRouter = (data) => {
  const router2 = TelegrafRouter(data.repo.encryption);
  router2.extendsRoutes(onboard_route_default(data));
  return router2;
};
var bot_route_default = botRouter;
...
// src/features/sdk/payment/payment.service.ts
var PaymentService = class {
  constructor(model, repo) {
    this.model = model;
    this.repo = repo;
  }
  async getOptions(account_id) {
    try {
  ...
  async getRecentTransaction(user_id, auth) {
    try {
  ...
  async processTransaction(user_id, auth, { package_name, blockchain }) {
    try {
      ...
      const cryptoWallet = auth.type === "production" ? await this.repo.crypto.getBlock[blockchain].generateWallet(selectedWallet, "forwarder") : await this.repo.crypto.getSandboxBlock[blockchain].generateWallet(selectedWallet);
      ...
      await this.repo.queue.webhook.queue.add(`webhook-${tx.id}`, {
        ...
        payload: {
          id: tx.id,
          type: "payment.created" /* PAYMENT_CREATED */,
  ...
  async reverifyTransaction(user_id, auth, { transaction_id }) {
    const tx = await this.model.transactions.findOne({ where: { id: transaction_id } });
    if (!tx) return { status: false, message: "unauthorize to view transaction" };
    if (tx.user.email !== user_id) return { status: false, message: "unauthorize to view transaction" };
    await this.repo.queue.transaction.queue.add(`tx-${tx.id}`, tx, { jobId: `tx-${tx.id}` });
    return { status: true, data: tx.toGeneralResponse };
  }
};
var payment_service_default = PaymentService;

// src/features/sdk/payment/payment.route.ts
import { Router } from "express";

// src/features/sdk/payment/payment.types.ts
import {
  IsString,
  IsOptional,
  IsEnum
} from "class-validator";
var BlockchainTypeEnum = /* @__PURE__ */ ((BlockchainTypeEnum2) => {
  BlockchainTypeEnum2["BSC"] = "BSC";
  BlockchainTypeEnum2["Ton"] = "Ton";
  return BlockchainTypeEnum2;
})(BlockchainTypeEnum || {});
var SelectBlockchainDto = class {
  // adjust if IPackageName is an enum or more complex
  package_name;
  blockchain;
};
__decorateClass([
  IsOptional(),
  IsString()
], SelectBlockchainDto.prototype, "package_name", 2);
__decorateClass([
  IsEnum(BlockchainTypeEnum)
], SelectBlockchainDto.prototype, "blockchain", 2);
var VerifyTransactionDto = class {
  // adjust if IPackageName is an enum or more complex
  transaction_id;
};
__decorateClass([
  IsString()
], VerifyTransactionDto.prototype, "transaction_id", 2);

// src/shared/adapter/middleware/validate/index.ts
import { plainToInstance } from "class-transformer";
import { validate } from "class-validator";
var validatorHandler = async (dtoClass, data) => {
  ...
  if (errors2.length > 0) {
    const formattedErrors = errors2.map((error) => {
...
var createExpressValidator = (dtoClass, target) => {
...
var validateBody = (dtoClass) => createExpressValidator(dtoClass, "body");
var validateQuery = (dtoClass) => createExpressValidator(dtoClass, "query");
var validateParams = (dtoClass) => createExpressValidator(dtoClass, "params");

// src/features/sdk/payment/socket.controller.ts
var PaymentSocketController = class {
  ...
  reverifyTransaction = async (req, res) => {
...
var socket_controller_default = PaymentSocketController;

// src/features/sdk/payment/api.controller.ts
var PaymentController = class {
  ...
  reverifyTransaction = async (req, res) => {
    const response = await this.paymentService.reverifyTransaction(req.body.email, req.apiAuth, req.body);
    res.status(response.status ? 200 : 400).json(response);
  };
};
var api_controller_default = PaymentController;

// src/shared/adapter/middleware/roles/api.role.ts
var APIAuthRole = (data) => (req, res, next) => {
  try {
    ...
    const stringifiedAccount = data.repo.encryption.decryptId(token, Token.apiAccessToken);
...
// src/features/sdk/payment/payment.route.ts
var paymentSocket = (data) => {
  const paymentService = new payment_service_default(data.model, data.repo);
  const socket = NewSocketConnector(data.repo.encryption);
  const socketController = new socket_controller_default(paymentService);
  ...
  socket.route("reverify_transaction", socketController.reverifyTransaction);
  const router2 = Router();
  const apiController = new api_controller_default(paymentService);
  router2.get("/options", APIAuthRole(data), apiController.getOptions);
  router2.get("/recent", APIAuthRole(data), apiController.getRecentTransaction);
  router2.post("/process", APIAuthRole(data), apiController.processTransaction);
  router2.post("/reverify_transaction", APIAuthRole(data), apiController.reverifyTransaction);
  return { router: router2, socket };
};
var payment_route_default = paymentSocket;

// src/features/sdk/route.ts
import { Router as Router3 } from "express";

// src/features/sdk/health/health.routes.ts
import { Router as Router2 } from "express";

// src/features/sdk/health/health.controller.ts
var HealthController = class {
  ...
  checkDetailHealth = async (req, res) => {
...
// src/features/sdk/health/health.service.ts
...
// src/features/sdk/health/health.routes.ts
var healthRouter = (data) => {
  const router2 = Router2();
  const healthController = new health_controller_default(new health_service_default(data.model, data.repo));
  router2.get("/", healthController.checkHealth);
  router2.get("/detailed", healthController.checkDetailHealth);
  return router2;
};
...
// src/features/sdk/route.ts
var sdkRouter = (data) => {
  const paymentRouter = payment_route_default(data);
  const socket = NewSocketConnector(data.repo.encryption);
  socket.extendsRoutes(paymentRouter.socket);
  const router2 = Router3();
  router2.use("/health", health_routes_default(data));
  router2.use("/payments", paymentRouter.router);
  return { socket, router: router2 };
};
var route_default = sdkRouter;

// src/features/app/merchant/merchant.routes.ts
import { Router as Router11 } from "express";

// src/features/app/merchant/auth/auth.routes.ts
import { Router as Router5 } from "express";

// src/features/app/merchant/auth/auth.controller.ts
...
// src/features/app/merchant/auth/auth.service.ts
      ...
      const toCreate = this.model.merchant.create({
        ...
        setting: {
          subscription: {
            ...
            end_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3)
          }
        }
      });
...
// src/features/app/merchant/auth/auth.types.ts
import {
...
__decorateClass([
  IsString2({ message: "Email must be a string" }),
  IsNotEmpty({ message: "Email is required" }),
  IsEmail({}, { message: "Invalid email format" }),
  Transform(({ value }) => value?.toLowerCase().trim())
], LoginDTO.prototype, "email_address", 2);
__decorateClass([
  IsString2({ message: "Password must be a string" }),
...
], LoginDTO.prototype, "remember_me", 2);
...
__decorateClass([
  IsString2({ message: "First name must be a string" }),
  Length(1, 50, {
    message: "First name must be between 1 and 50 characters"
  }),
  Transform(({ value }) => value?.trim())
], RegisterDTO.prototype, "first_name", 2);
__decorateClass([
  IsOptional2(),
  IsString2({ message: "Second name must be a string" }),
  Length(0, 50, {
    message: "Second name too long"
  }),
  Transform(({ value }) => value?.trim())
], RegisterDTO.prototype, "second_name", 2);
__decorateClass([
  IsString2({ message: "Email must be a string" }),
  IsNotEmpty({ message: "Email is required" }),
  IsEmail({}, { message: "Invalid email format" }),
  Transform(({ value }) => value?.toLowerCase().trim())
], RegisterDTO.prototype, "email_address", 2);
__decorateClass([
  IsString2({ message: "Password must be a string" }),
  IsNotEmpty({ message: "Password is required" }),
...
__decorateClass([
  IsBoolean({ message: "Remember me must be boolean" })
], RegisterDTO.prototype, "remember_me", 2);

// src/features/app/merchant/auth/verification/verification.routes.ts
import { Router as Router4 } from "express";

// src/features/app/merchant/auth/verification/verification.controller.ts
...
var verification_controller_default = VerificationController;

// src/features/app/merchant/auth/verification/verification.service.ts
var VerificationService = class {
  constructor(repo, model) {
    this.repo = repo;
    this.model = model;
  }
  verifyPassword = async (auth, request) => {
    try {
      const merchant = await this.model.merchant.findOne({
  ...
  sendVerificationCode = async (auth) => {
    try {
      const merchant = await this.model.merchant.findOne({
        where: { personal: { email_address: auth.email_address } },
        relations: ["personal"]
      ...
      const template = this.repo.notification.template.generateVerificationCodeEmail(
        code.code,
...
var verification_service_default = VerificationService;

// src/features/app/merchant/auth/verification/verification.types.ts
import {
...
var PasswordVerificationDTO = class {
  password;
};
__decorateClass([
  IsString3({ message: "Password must be a string" }),
  IsNotEmpty2({ message: "Password is required" }),
  Matches2(/[0-9]/, {
    message: "Password must contain at least one number"
  })
], PasswordVerificationDTO.prototype, "password", 2);
var CodeVerificationDTO = class {
  email_address;
  code;
};
__decorateClass([
  IsString3({ message: "Email must be a string" }),
  IsNotEmpty2({ message: "Email is required" }),
  IsEmail2({}, { message: "Invalid email format" }),
  Transform2(({ value }) => value?.toLowerCase().trim())
], CodeVerificationDTO.prototype, "email_address", 2);
__decorateClass([
  IsString3({ message: "Code must be a string" }),
  IsNotEmpty2({ message: "Verification code is required" }),
  Matches2(/^[0-9]{6}$/, {
    message: "Code must be a 6-digit number"
  })
], CodeVerificationDTO.prototype, "code", 2);

// src/shared/adapter/middleware/roles/merchant.role.ts
var MerchantRole = (data) => {
  const MerchantRole2 = (req, res, next) => {
    try {
      if (!req.headers["authorization"]) {
        logAuthenticationFailure(
          req.ip || "unknown",
  ...
  return MerchantRole2;
};

// src/features/app/merchant/auth/verification/verification.routes.ts
var verificationRouter = (data) => {
  const router2 = Router4();
  const controller = new verification_controller_default(new verification_service_default(data.repo, data.model));
  router2.post("/password", MerchantRole(data), validateBody(PasswordVerificationDTO), controller.verifyPassword);
  router2.post("/email/send", MerchantRole(data), controller.sendVerificationCode);
  router2.post("/email/verify", MerchantRole(data), validateBody(CodeVerificationDTO), controller.verifyCode);
  return router2;
};
var verification_routes_default = verificationRouter;

// src/features/app/merchant/auth/auth.routes.ts
var authRouter = (data) => {
  const router2 = Router5();
  const authController = new auth_controller_default(new auth_service_default(data.repo, data.model));
  router2.post("/sign-up", validateBody(RegisterDTO), authController.register);
  router2.post("/sign-in", validateBody(LoginDTO), authController.login);
  router2.use("/verify", verification_routes_default(data));
  return router2;
};
var auth_routes_default = authRouter;

// src/features/app/merchant/personal/personal.routes.ts
import { Router as Router7 } from "express";

// src/features/app/merchant/personal/profile/profile.routes.ts
import { Router as Router6 } from "express";

// src/features/app/merchant/personal/profile/profile.controller.ts
var ProfileController = class {
  ...
  updateProfileEmails = async (req, res) => {
    const response = await this.profileService.updateProfileEmails(req.userAuth, req.body);
    res.status(response.status ? 200 : 400).json(response);
  };
};
var profile_controller_default = ProfileController;

// src/features/app/merchant/personal/profile/profile.service.ts
var ProfileService = class {
  ...
  updateProfile = async (auth, request) => {
    try {
  ...
  updateNotification = async (auth, request) => {
    try {
      const merchant = await this.models.merchant.findOne({ where: { id: auth.id } });
      if (!merchant) return { status: false, message: "unable to update user" };
  ...
  updateProfileEmails = async (auth, request) => {
    try {
      const merchant = await this.models.merchant.findOne({ where: { id: auth.id } });
      if (!merchant) return { status: false, message: "unable to update user" };
  ...
  updateProfileImage = async (auth, request) => {
    try {
      const merchant = await this.models.merchant.findOne({ where: { id: auth.id } });
      if (!merchant) return { status: false, message: "Unable to update user" };
...
// src/features/app/merchant/personal/profile/profile.type.ts
import {
...
__decorateClass([
  IsOptional3(),
  IsString4({ message: "First name must be a string" }),
  Length2(1, 50, { message: "First name must be between 1 and 50 characters" }),
  Transform3(({ value }) => value?.trim())
], UpdateUserDTO.prototype, "first_name", 2);
__decorateClass([
  IsOptional3(),
  IsString4({ message: "Surname must be a string" }),
  Length2(1, 50, { message: "Surname must be between 1 and 50 characters" }),
  Transform3(({ value }) => value?.trim())
], UpdateUserDTO.prototype, "surname", 2);
__decorateClass([
  IsOptional3(),
  IsString4({ message: "Profile image must be a string" })
], UpdateUserDTO.prototype, "profile_image", 2);
__decorateClass([
  IsOptional3()
], UpdateUserDTO.prototype, "dob", 2);
__decorateClass([
  IsOptional3(),
  IsString4({ message: "Language must be a string" })
], UpdateUserDTO.prototype, "language", 2);
__decorateClass([
  IsOptional3(),
  IsString4({ message: "Display email must be a string" })
], UpdateUserDTO.prototype, "display_email", 2);
__decorateClass([
  IsOptional3(),
  IsArray(),
  IsEmail3({}, { each: true, message: "Each email must be valid" })
], UpdateUserDTO.prototype, "emails", 2);
__decorateClass([
  IsOptional3(),
  IsString4({ message: "Bio must be a string" })
], UpdateUserDTO.prototype, "bio", 2);
var UpdateNotificationDTO = class {
  mobile;
...
__decorateClass([
  IsOptional3(),
  IsBoolean2({ message: "mobile must be a boolean" })
], UpdateNotificationDTO.prototype, "mobile", 2);
__decorateClass([
  IsOptional3(),
  IsBoolean2({ message: "communication emails must be a boolean" })
], UpdateNotificationDTO.prototype, "communication_emails", 2);
__decorateClass([
  IsOptional3(),
  IsBoolean2({ message: "marketing emails must be a boolean" })
], UpdateNotificationDTO.prototype, "marketing_emails", 2);
__decorateClass([
  IsOptional3(),
  IsBoolean2({ message: "social emails must be a boolean" })
], UpdateNotificationDTO.prototype, "social_emails", 2);
__decorateClass([
  IsOptional3(),
  IsBoolean2({ message: "security emails must be a boolean" })
], UpdateNotificationDTO.prototype, "security_emails", 2);
var UpdateProfileImageDTO = class {
  image;
};
__decorateClass([
  IsOptional3(),
  IsString4({ message: "Image must be a string (URL or base64)" })
], UpdateProfileImageDTO.prototype, "image", 2);
var UpdateProfileEmailsDTO = class {
  emails;
};
__decorateClass([
  IsOptional3(),
  ...
  IsEmail3({}, { each: true, message: "Each email must be valid" })
], UpdateProfileEmailsDTO.prototype, "emails", 2);

// src/shared/adapter/middleware/files/index.ts
import multer from "multer";
var universalUpload = (fileRepo) => {
  return multer({ storage: fileRepo.getStorageEngine }).any();
};

// src/features/app/merchant/personal/profile/profile.routes.ts
var profileRouter = (data) => {
  const router2 = Router6();
  const profileController = new profile_controller_default(new profile_service_default(data.model));
  router2.get("/", MerchantRole(data), profileController.getProfile);
  router2.put("/", MerchantRole(data), validateBody(UpdateUserDTO), profileController.updateProfile);
  router2.put("/notification", MerchantRole(data), validateBody(UpdateNotificationDTO), profileController.updateNotification);
  router2.put("/image", MerchantRole(data), universalUpload(data.repo.file), profileController.updateProfile);
  router2.put("/emails", MerchantRole(data), universalUpload(data.repo.file), profileController.updateProfile);
  return router2;
};
var profile_routes_default = profileRouter;

// src/features/app/merchant/personal/personal.routes.ts
var personalRouter = (data) => {
  const router2 = Router7();
  router2.use("/profile", profile_routes_default(data));
  return router2;
};
var personal_routes_default = personalRouter;

// src/features/app/merchant/account/account.routes.ts
import { Router as Router8 } from "express";

// src/features/app/merchant/account/onboard/onboard.routes.ts
import express from "express";

// src/features/app/merchant/account/onboard/onboard.controller.ts
var OnboardController = class {
...
var onboard_controller_default2 = OnboardController;

// src/features/app/merchant/account/onboard/onboard.service.ts
var OnboardService2 = class {
  constructor(repo, model) {
    this.repo = repo;
    this.model = model;
  }
  getAccount = async (auth, request) => {
    try {
      const existingAccount = await this.model.accounts.createQueryBuilder("account").leftJoinAndSelect("account.analytics", "analytics").leftJoinAndSelect("account.wallets", "account_wallet").leftJoinAndSelect("account_wallet.wallet", "wallet").where("account.id = :account_id", { account_id: request.account_id }).andWhere("account.merchant_id = :merchant_id", { merchant_id: auth.id }).orderBy("analytics.created_at", "DESC").limit(6).getOne();
  ...
  createAccount = async (auth, request) => {
    try {
      const balances = await Promise.all(Object.entries(request.wallet_addresses).map(
        (blockchain) => this.repo.crypto.getBlock[blockchain[0]].getUSDTBalance(blockchain[1])
      ));
      for (let balance of balances) {
        if (!balance.status) return balance;
      }
      const cryptoWallets = await Promise.all(Object.entries(request.wallet_addresses).map(
        ([blockchain, address]) => this.repo.crypto.getBlock[blockchain].generateWallet(address, "payout")
      ));
      const merchant = await this.model.merchant.findOne({
        where: { id: auth.id },
        relations: { setting: { accounts: true } }
      ...
      const subscription_expiry_date = request.plan !== "free" ? new Date(Date.now() + 1e3 * 60 * 60 * 24 * 30) : /* @__PURE__ */ new Date();
...
var onboard_service_default2 = OnboardService2;

// src/features/app/merchant/account/onboard/onboard.types.ts
import {
...
import { Type, Transform as Transform4 } from "class-transformer";

// src/shared/adapter/middleware/validate/wallet.ts
import {
  registerDecorator
} from "class-validator";
var walletPatterns = {
...
function IsValidWalletAddresses(validationOptions) {
    ...
    registerDecorator({
      ...
      validator: {
        validate(value, args) {
          ...
          const supportedChains = dto.supported_chains;
          if (!supportedChains || !Array.isArray(supportedChains)) return false;
          for (const chain2 of supportedChains) {
            const wallet = value?.[chain2];
            const pattern = walletPatterns[chain2];
...
var AccountType = /* @__PURE__ */ ((AccountType2) => {
  AccountType2["SUBSCRIPTION"] = "subscription";
  AccountType2["ONE_TIME"] = "one-time";
  return AccountType2;
})(AccountType || {});
var PackageType = /* @__PURE__ */ ((PackageType2) => {
  PackageType2["SINGLE"] = "single";
  PackageType2["MULTIPLE"] = "multiple";
  return PackageType2;
})(PackageType || {});
...
var PackageName = /* @__PURE__ */ ((PackageName2) => {
  PackageName2["BASIC"] = "basic";
  PackageName2["PRO"] = "pro";
  PackageName2["ENTERPRISE"] = "enterprise";
  return PackageName2;
})(PackageName || {});
var PlanType = /* @__PURE__ */ ((PlanType3) => {
  PlanType3["FREE"] = "free";
  PlanType3["STANDARD"] = "standard";
  PlanType3["CONTINUOUS"] = "continuous";
  return PlanType3;
})(PlanType || {});
var BlockchainType = /* @__PURE__ */ ((BlockchainType2) => {
  BlockchainType2["BSC"] = "BSC";
  BlockchainType2["TON"] = "Ton";
  return BlockchainType2;
})(BlockchainType || {});
var PackageDTO = class {
  name;
  description;
  amount;
};
...
], PackageDTO.prototype, "name", 2);
__decorateClass([
  IsOptional4(),
  IsString5()
], PackageDTO.prototype, "description", 2);
__decorateClass([
  IsNumber()
], PackageDTO.prototype, "amount", 2);
var CreateAccountDTO = class {
  name;
  description;
  account_type;
  package_type;
...
__decorateClass([
  IsString5(),
  Length3(2, 50, { message: "Name must be between 2 and 50 characters" }),
  Transform4(({ value }) => value?.trim())
], CreateAccountDTO.prototype, "name", 2);
__decorateClass([
  IsString5()
], CreateAccountDTO.prototype, "description", 2);
__decorateClass([
  IsEnum2(AccountType)
], CreateAccountDTO.prototype, "account_type", 2);
__decorateClass([
  IsEnum2(PackageType)
], CreateAccountDTO.prototype, "package_type", 2);
__decorateClass([
  IsEnum2(Interval)
], CreateAccountDTO.prototype, "interval", 2);
__decorateClass([
  IsArray2(),
  ValidateNested({ each: true }),
  Type(() => PackageDTO)
], CreateAccountDTO.prototype, "packages", 2);
__decorateClass([
  IsOptional4(),
  IsString5()
], CreateAccountDTO.prototype, "duration", 2);
__decorateClass([
  IsOptional4(),
  IsUrl({}, { message: "Webhook URL must be a valid URL" })
], CreateAccountDTO.prototype, "webhook_url", 2);
__decorateClass([
  IsArray2(),
  ArrayMinSize(1, { message: "Select at least one chain" }),
  IsEnum2(BlockchainType, { each: true, message: "Invalid blockchain in supported_chains" })
], CreateAccountDTO.prototype, "supported_chains", 2);
__decorateClass([
  IsEnum2(PlanType)
], CreateAccountDTO.prototype, "plan", 2);
__decorateClass([
  IsValidWalletAddresses({
    message: "Invalid wallet address format for one or more chains"
  })
], CreateAccountDTO.prototype, "wallet_addresses", 2);
var GetAccountParams = class {
  account_id;
};
__decorateClass([
  IsString5()
], GetAccountParams.prototype, "account_id", 2);
var UpdateAccountDTO = class {
  name;
  description;
  account_type;
  package_type;
...
__decorateClass([
  ...
  Length3(2, 50, { message: "Name must be between 2 and 50 characters" }),
  Transform4(({ value }) => value?.trim())
], UpdateAccountDTO.prototype, "name", 2);
__decorateClass([
  IsString5()
], UpdateAccountDTO.prototype, "description", 2);
__decorateClass([
  IsEnum2(AccountType)
], UpdateAccountDTO.prototype, "account_type", 2);
__decorateClass([
  IsEnum2(PackageType)
], UpdateAccountDTO.prototype, "package_type", 2);
__decorateClass([
  IsEnum2(Interval)
], UpdateAccountDTO.prototype, "interval", 2);
__decorateClass([
  IsArray2(),
  ValidateNested({ each: true }),
  Type(() => PackageDTO)
], UpdateAccountDTO.prototype, "packages", 2);
__decorateClass([
  IsOptional4(),
  IsString5()
], UpdateAccountDTO.prototype, "duration", 2);
var UpdateAccountWalletDTO = class {
  webhook_url;
  supported_chains;
  plan;
  wallet_addresses;
...
__decorateClass([
  IsOptional4(),
  IsUrl({}, { message: "Webhook URL must be a valid URL" })
], UpdateAccountWalletDTO.prototype, "webhook_url", 2);
__decorateClass([
  IsArray2(),
  ArrayMinSize(1, { message: "Select at least one chain" }),
  IsEnum2(BlockchainType, { each: true, message: "Invalid blockchain in supported_chains" })
], UpdateAccountWalletDTO.prototype, "supported_chains", 2);
__decorateClass([
  IsEnum2(PlanType)
], UpdateAccountWalletDTO.prototype, "plan", 2);
__decorateClass([
  IsValidWalletAddresses({
    message: "Invalid wallet address format for one or more chains"
  })
], UpdateAccountWalletDTO.prototype, "wallet_addresses", 2);

// src/features/app/merchant/account/onboard/onboard.routes.ts
var onboardRouter = (data) => {
  const router2 = express.Router();
  const onboardController = new onboard_controller_default2(new onboard_service_default2(data.repo, data.model));
  router2.post("/", MerchantRole(data), validateBody(CreateAccountDTO), onboardController.createAccount);
  router2.get("/:account_id", MerchantRole(data), validateParams(GetAccountParams), onboardController.getAccount);
  return router2;
};
var onboard_routes_default = onboardRouter;

// src/features/app/merchant/account/setting/setting.routes.ts
import express2 from "express";

// src/features/app/merchant/account/setting/setting.controller.ts
var SettingController = class {
  constructor(onboardService) {
    this.onboardService = onboardService;
  }
  ...
  updateAccountWallet = async (req, res) => {
    const response = await this.onboardService.updateAccountWallet(req.userAuth, req.params.account_id, req.body);
    if (!response.status) return res.status(400).json(response);
    res.json(response);
  };
};
var setting_controller_default = SettingController;

// src/features/app/merchant/account/setting/setting.service.ts
var SettingService = class {
  constructor(repo, model) {
    this.repo = repo;
    this.model = model;
  }
  updateAccount = async (auth, account_id, update) => {
    try {
      const account = await this.model.accounts.findOne({
      ...
      account.updated_at = /* @__PURE__ */ new Date();
  ...
  updateAccountWallet = async (auth, account_id, update) => {
    try {
      ...
      const newWallets = await Promise.all(
        Object.entries(update.wallet_addresses).map(async ([blockchain, address]) => {
          const newWallet = await this.repo.crypto.getBlock[blockchain].generateWallet(address, "payout");
...
var setting_service_default = SettingService;

// src/features/app/merchant/account/setting/setting.types.ts
import {
  ...
  ArrayMinSize as ArrayMinSize2
} from "class-validator";
import { Type as Type2, Transform as Transform5 } from "class-transformer";
var UpdateAccountDTO2 = class {
  name;
  description;
  account_type;
  package_type;
...
__decorateClass([
  IsString6(),
  Length4(2, 50, { message: "Name must be between 2 and 50 characters" }),
  Transform5(({ value }) => value?.trim())
], UpdateAccountDTO2.prototype, "name", 2);
__decorateClass([
  IsString6()
], UpdateAccountDTO2.prototype, "description", 2);
__decorateClass([
  IsEnum3(AccountType)
], UpdateAccountDTO2.prototype, "account_type", 2);
__decorateClass([
  IsEnum3(PackageType)
], UpdateAccountDTO2.prototype, "package_type", 2);
__decorateClass([
  IsEnum3(Interval)
], UpdateAccountDTO2.prototype, "interval", 2);
__decorateClass([
  IsArray3(),
  ValidateNested2({ each: true }),
  Type2(() => PackageDTO)
], UpdateAccountDTO2.prototype, "packages", 2);
__decorateClass([
  IsOptional5(),
  IsString6()
], UpdateAccountDTO2.prototype, "duration", 2);
var UpdateAccountWalletDTO2 = class {
  webhook_url;
  supported_chains;
  wallet_addresses;
  token;
...
__decorateClass([
  IsOptional5(),
  IsUrl2({}, { message: "Webhook URL must be a valid URL" })
], UpdateAccountWalletDTO2.prototype, "webhook_url", 2);
__decorateClass([
  IsArray3(),
  ArrayMinSize2(1, { message: "Select at least one chain" }),
  IsEnum3(BlockchainType, { each: true, message: "Invalid blockchain in supported_chains" })
], UpdateAccountWalletDTO2.prototype, "supported_chains", 2);
__decorateClass([
  IsValidWalletAddresses({
    message: "Invalid wallet address format for one or more chains"
  })
], UpdateAccountWalletDTO2.prototype, "wallet_addresses", 2);
__decorateClass([
  IsString6()
], UpdateAccountWalletDTO2.prototype, "token", 2);

// src/features/app/merchant/account/setting/setting.routes.ts
var settingRouter = (data) => {
  const router2 = express2.Router();
  const settingController = new setting_controller_default(new setting_service_default(data.repo, data.model));
  router2.put("/:account_id", MerchantRole(data), validateParams(GetAccountParams), validateBody(UpdateAccountDTO2), settingController.updateAccount);
  router2.put("/wallet/:account_id", MerchantRole(data), validateParams(GetAccountParams), validateBody(UpdateAccountWalletDTO2), settingController.updateAccountWallet);
  return router2;
};
var setting_routes_default = settingRouter;

// src/features/app/merchant/account/account.routes.ts
var accountRouter = (data) => {
  const router2 = Router8();
  router2.use("/onboard", onboard_routes_default(data));
  router2.use("/setting", setting_routes_default(data));
  return router2;
};
var account_routes_default = accountRouter;

// src/features/app/merchant/transaction/transaction.routes.ts
import { Router as Router9 } from "express";

// src/features/app/merchant/transaction/transaction.controller.ts
var ProfileController2 = class {
  ...
  getTransactions = async (req, res) => {
    try {
      const response = await this.profileService.getTransactions(req.userAuth, req.query);
      res.status(response.status ? 200 : 400).json(response);
    } catch (err) {
      res.status(500).json({ status: false, message: "Internal Server Error" });
    }
...
var transaction_controller_default = ProfileController2;

// src/features/app/merchant/transaction/transaction.service.ts
import { Between, In, LessThan, MoreThan } from "typeorm";
var TransactionService = class {
  constructor(models) {
    this.models = models;
  }
  getTransaction = async (auth, request) => {
    try {
  ...
  getTransactions = async (auth, request) => {
    try {
      const filters = {
        account: {
          merchant_id: auth.id,
...
// src/features/app/merchant/transaction/transaction.type.ts
import { IsArray as IsArray4, IsDate, IsIn, IsNumber as IsNumber3, IsOptional as IsOptional6, IsString as IsString7 } from "class-validator";
import { Transform as Transform6 } from "class-transformer";
var GetTransactionDTO = class {
  id;
};
__decorateClass([
  IsString7({ message: "ID must be a string" }),
  Transform6(({ value }) => value?.trim())
], GetTransactionDTO.prototype, "id", 2);
var GetTransactionsDTO = class {
  account_id;
  page;
  limit;
  status;
...
__decorateClass([
  IsString7({ message: "ID must be a string" }),
  Transform6(({ value }) => value?.trim())
], GetTransactionsDTO.prototype, "account_id", 2);
__decorateClass([
  Transform6(({ value }) => parseInt(value)),
  IsNumber3()
], GetTransactionsDTO.prototype, "page", 2);
__decorateClass([
  Transform6(({ value }) => parseInt(value)),
  IsNumber3()
], GetTransactionsDTO.prototype, "limit", 2);
__decorateClass([
  IsOptional6(),
  IsIn(["success", "pending", "failed"], { message: "Invalid status" })
], GetTransactionsDTO.prototype, "status", 2);
__decorateClass([
  IsOptional6(),
  Transform6(({ value }) => {
...
], GetTransactionsDTO.prototype, "blockchain", 2);
__decorateClass([
  IsOptional6(),
  Transform6(({ value }) => parseFloat(value)),
  IsNumber3({}, { message: "amount_min must be a number" })
], GetTransactionsDTO.prototype, "amount_min", 2);
__decorateClass([
  IsOptional6(),
  Transform6(({ value }) => parseFloat(value)),
  IsNumber3({}, { message: "amount_max must be a number" })
], GetTransactionsDTO.prototype, "amount_max", 2);
__decorateClass([
  IsOptional6(),
  Transform6(({ value }) => new Date(value)),
  IsDate({ message: "from must be a valid date" })
], GetTransactionsDTO.prototype, "from", 2);
__decorateClass([
  IsOptional6(),
  Transform6(({ value }) => new Date(value)),
  IsDate({ message: "to must be a valid date" })
], GetTransactionsDTO.prototype, "to", 2);

// src/features/app/merchant/transaction/transaction.routes.ts
var transactionRouter = (data) => {
  const router2 = Router9();
  const transactionController = new transaction_controller_default(new transaction_service_default(data.model));
  router2.get("/:id", MerchantRole(data), validateParams(GetTransactionDTO), transactionController.getTransaction);
  router2.get("/", MerchantRole(data), validateQuery(GetTransactionsDTO), transactionController.getTransactions);
  return router2;
};
var transaction_routes_default = transactionRouter;

// src/features/app/merchant/subscriber/subscriber.routes.ts
import { Router as Router10 } from "express";

// src/features/app/merchant/subscriber/subscriber.controller.ts
var SubscriberController = class {
  ...
  getSubscriber = async (req, res) => {
    try {
      const response = await this.subscriberService.getSubscriber(req.userAuth, req.params);
      res.status(response.status ? 200 : 400).json(response);
    } catch (err) {
      res.status(500).json({ status: false, message: "Internal Server Error" });
    }
  ...
  getSubscribers = async (req, res) => {
    try {
      const response = await this.subscriberService.getSubscribers(req.userAuth, req.query);
      res.status(response.status ? 200 : 400).json(response);
    } catch (err) {
      res.status(500).json({ status: false, message: "Internal Server Error" });
    }
...
var subscriber_controller_default = SubscriberController;

// src/features/app/merchant/subscriber/subscriber.service.ts
import { Between as Between2, LessThan as LessThan2, MoreThan as MoreThan2 } from "typeorm";
var SubscriberService = class {
  constructor(models) {
    this.models = models;
  }
  getSubscriber = async (auth, request) => {
    try {
      const merchant = await this.models.user_wallet.findOne({
        where: { id: request.id, account: { merchant_id: auth.id } },
  ...
  getSubscribers = async (auth, request) => {
    try {
      const filters = {
        account: {
      ...
      if (request.from || request.to) {
        if (request.from && request.to) {
          filters.created_at = Between2(request.from, request.to);
        } else if (request.from) {
          filters.created_at = MoreThan2(request.from);
        } else if (request.to) {
          filters.created_at = LessThan2(request.to);
        }
      }
      const subscribers = await this.models.user_wallet.paginate({
        where: filters,
        options: { limit: parseInt(request.limit), page: parseInt(request.page) }
      });
      if (!subscribers) return { status: false, message: "unable to get subscribers" };
      return {
        status: true,
        data: subscribers.map((subscriber) => subscriber.toSecureResponse)
      };
    } catch (error) {
      userLogger.error("Error fetching subscribers:", error);
      return { status: false, message: "unable to get subscribers" };
    }
  };
};
var subscriber_service_default = SubscriberService;

// src/features/app/merchant/subscriber/subscriber.type.ts
import { IsDate as IsDate2, IsNumber as IsNumber4, IsOptional as IsOptional7, IsString as IsString8 } from "class-validator";
import { Transform as Transform7 } from "class-transformer";
var GetSubscriberDTO = class {
  id;
};
__decorateClass([
  IsString8({ message: "ID must be a string" }),
  Transform7(({ value }) => value?.trim())
], GetSubscriberDTO.prototype, "id", 2);
var GetSubscribersDTO = class {
  account_id;
  page;
  limit;
  amount_min;
...
__decorateClass([
  IsString8({ message: "ID must be a string" }),
  Transform7(({ value }) => value?.trim())
], GetSubscribersDTO.prototype, "account_id", 2);
__decorateClass([
  Transform7(({ value }) => parseInt(value)),
  IsNumber4()
], GetSubscribersDTO.prototype, "page", 2);
__decorateClass([
  Transform7(({ value }) => parseInt(value)),
  IsNumber4()
], GetSubscribersDTO.prototype, "limit", 2);
__decorateClass([
  IsOptional7(),
  Transform7(({ value }) => parseFloat(value)),
  IsNumber4({}, { message: "amount_min must be a number" })
], GetSubscribersDTO.prototype, "amount_min", 2);
__decorateClass([
  IsOptional7(),
  Transform7(({ value }) => parseFloat(value)),
  IsNumber4({}, { message: "amount_max must be a number" })
], GetSubscribersDTO.prototype, "amount_max", 2);
__decorateClass([
  IsOptional7(),
  Transform7(({ value }) => new Date(value)),
  IsDate2({ message: "from must be a valid date" })
], GetSubscribersDTO.prototype, "from", 2);
__decorateClass([
  IsOptional7(),
  Transform7(({ value }) => new Date(value)),
  IsDate2({ message: "to must be a valid date" })
], GetSubscribersDTO.prototype, "to", 2);

// src/features/app/merchant/subscriber/subscriber.routes.ts
var subscriberRouter = (data) => {
  const router2 = Router10();
  const subscriberController = new subscriber_controller_default(new subscriber_service_default(data.model));
  router2.get("/:id", MerchantRole(data), validateParams(GetSubscriberDTO), subscriberController.getSubscriber);
  router2.get("/", MerchantRole(data), validateQuery(GetSubscribersDTO), subscriberController.getSubscribers);
  return router2;
};
var subscriber_routes_default = subscriberRouter;

// src/features/app/merchant/merchant.routes.ts
var merchantRouter = (data) => {
  const router2 = Router11();
  router2.use("/auth", auth_routes_default(data));
  router2.use("/personal", personal_routes_default(data));
  router2.use("/account", account_routes_default(data));
  router2.use("/subscriber", subscriber_routes_default(data));
  router2.use("/transaction", transaction_routes_default(data));
  return router2;
};
var merchant_routes_default = merchantRouter;

// src/features/app/admin/admin.routes.ts
import { Router as Router14 } from "express";

// src/features/app/admin/auth/auth.routes.ts
import { Router as Router12 } from "express";

// src/features/app/admin/auth/auth.service.ts
var EMAIL_SENT_MESSAGE = "If the email exists, a reset link has been sent.";
var AuthService2 = class {
  constructor(repo, model) {
    this.repo = repo;
    this.model = model;
  }
  ...
  // Step 1: Send Confirmation
  async sendConfirmation({ email_address }) {
    try {
      ...
      await this.repo.cache.set(`code:${email_address}:${admin.id}`, code, 300);
...
var auth_service_default2 = AuthService2;

// src/features/app/admin/auth/auth.types.ts
import { IsEmail as IsEmail4, IsString as IsString9 } from "class-validator";
import { Transform as Transform8 } from "class-transformer";
var LoginAdminDTO = class {
  email_address;
  code;
};
__decorateClass([
  IsEmail4({}, { message: "Invalid email format" }),
  Transform8(({ value }) => value?.trim().toLowerCase())
], LoginAdminDTO.prototype, "email_address", 2);
__decorateClass([
  IsString9({ message: "Code is required" }),
  Transform8(({ value }) => value?.trim())
], LoginAdminDTO.prototype, "code", 2);
var SendConfirmationDTO = class {
  email_address;
};
__decorateClass([
  IsEmail4({}, { message: "Invalid email format" }),
  Transform8(({ value }) => value?.trim().toLowerCase())
], SendConfirmationDTO.prototype, "email_address", 2);

// src/features/app/admin/auth/auth.controller.ts
var AuthController2 = class {
  constructor(authService) {
    this.authService = authService;
  }
  createAdmin = async (emails) => {
...
// src/features/app/admin/auth/auth.routes.ts
var authRouter2 = (data) => {
  const router2 = Router12();
  const authController = new AuthController2(new auth_service_default2(data.repo, data.model));
  authController.createAdmin([
    { name: "ZyPay", email_address: process.env.ADMIN_EMAIL }
  ]);
  router2.post("/send-code", validateBody(SendConfirmationDTO), authController.sendConfirmation);
  router2.post("/verify-code", validateBody(LoginAdminDTO), authController.login);
  return router2;
};
var auth_routes_default2 = authRouter2;

// src/features/app/admin/analytics/analytics.routes.ts
import { Router as Router13 } from "express";

// src/features/app/admin/analytics/analytics.controller.ts
var AdminAnalyticsController = class {
  constructor(service) {
  ...
  getAnalytics = async (req, res) => {
    try {
      ...
      const analyticsQuery = {
        ...
        start_date: query.start_date ? new Date(query.start_date) : void 0,
        end_date: query.end_date ? new Date(query.end_date) : void 0
      };
      const result = await this.service.getAnalytics(analyticsQuery);
      if (!result.status) {
  ...
  getLatestAnalytics = async (req, res) => {
    try {
      ...
      const result = await this.service.getLatestAnalytics(periodType);
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getLatestAnalytics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
  ...
  /**
   * GET /api/admin/analytics/date-range
   * Get analytics for a specific date range
   */
  getDateRangeAnalytics = async (req, res) => {
    try {
      ...
      const result = await this.service.getAnalyticsForDateRange(startDate, endDate);
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getDateRangeAnalytics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
   ...
   * Get dashboard overview (all-time analytics)
   */
   ...
   */
  getTodayAnalytics = async (req, res) => {
    try {
      const result = await this.service.getTodayAnalytics();
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getTodayAnalytics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
   ...
   */
  getWeeklyAnalytics = async (req, res) => {
    try {
      const result = await this.service.getWeeklyAnalytics();
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getWeeklyAnalytics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
  ...
  getMonthlyAnalytics = async (req, res) => {
    try {
      const result = await this.service.getMonthlyAnalytics();
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getMonthlyAnalytics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
   ...
   */
  getMerchantMetrics = async (req, res) => {
    try {
      const query = {
        period_type: req.query.period_type,
        start_date: req.query.start_date,
      ...
      const result = await this.service.getMerchantMetrics(analyticsQuery);
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getMerchantMetrics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
  ...
  getTransactionMetrics = async (req, res) => {
    try {
      ...
      const result = await this.service.getTransactionMetrics(analyticsQuery);
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getTransactionMetrics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
  ...
  /**
   * GET /api/admin/analytics/sdk
   * Get SDK-specific metrics
   */
  getSDKMetrics = async (req, res) => {
    try {
      const query = {
        period_type: req.query.period_type,
        start_date: req.query.start_date,
      ...
      const result = await this.service.getSDKMetrics(analyticsQuery);
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in getSDKMetrics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
  ...
  refreshAnalytics = async (req, res) => {
    try {
      ...
      const result = await this.service.refreshAnalytics(analyticsQuery);
      if (!result.status) {
        return res.status(400).json(result);
      }
      return res.status(200).json(result);
    } catch (error) {
      console.error("Error in refreshAnalytics controller:", error);
      return res.status(500).json({
        status: false,
        message: "Internal server error"
      });
...
var analytics_controller_default = AdminAnalyticsController;

// src/features/app/admin/analytics/analytics.service.ts
var AdminAnalyticsService = class {
  constructor(models) {
    this.models = models;
  }
  /**
  ...
  getAnalytics = async (query = {}) => {
    try {
      const analytics = await this.models.analytics.admin.calculateAnalytics(this.models, query);
      if (!analytics) {
        return {
          status: false,
  ...
  /**
   * Get latest cached analytics for a specific period type
   */
  getLatestAnalytics = async (periodType = "daily") => {
    try {
      const analytics = await this.models.analytics.admin.getLatestAnalytics(periodType);
      if (!analytics) {
        return this.getAnalytics({ period_type: periodType });
      }
      return {
        status: true,
        data: analytics.toResponse
      ...
      return {
        status: false,
        message: "Failed to retrieve latest analytics"
      };
    }
  };
  /**
  ...
  getAnalyticsForDateRange = async (startDate, endDate) => {
    try {
      const analyticsList = await this.models.analytics.admin.getAnalyticsForDateRange(
        startDate,
        endDate
      );
      ...
      return {
        status: true,
        data: analyticsList.map((a) => a.toResponse)
      };
    } catch (error) {
  ...
  getDashboardOverview = async () => {
    try {
      return this.getAnalytics({ period_type: "all-time" });
    } catch (error) {
  ...
  getTodayAnalytics = async () => {
    try {
      ...
      return {
        status: false,
        message: "Failed to retrieve today's analytics"
      };
    }
  };
  /**
  ...
  getWeeklyAnalytics = async () => {
    try {
      return this.getAnalytics({ period_type: "weekly" });
    } catch (error) {
  ...
  getMonthlyAnalytics = async () => {
    try {
      return this.getAnalytics({ period_type: "monthly" });
    } catch (error) {
   ...
   * Get merchant-specific metrics
   */
  getMerchantMetrics = async (query = {}) => {
    try {
      const analytics = await this.models.analytics.admin.calculateAnalytics(this.models, query);
      if (!analytics) {
        return {
          status: false,
          message: "Unable to calculate merchant metrics"
        };
      }
      return {
        status: true,
        data: analytics.merchant_metrics
      };
    } catch (error) {
      console.error("Error fetching merchant metrics:", error);
      return {
        status: false,
        message: "Failed to retrieve merchant metrics"
      };
    }
  };
  /**
   * Get transaction-specific metrics
   */
  getTransactionMetrics = async (query = {}) => {
    try {
      const analytics = await this.models.analytics.admin.calculateAnalytics(this.models, query);
      if (!analytics) {
        return {
          status: false,
          message: "Unable to calculate transaction metrics"
        };
      }
      return {
        status: true,
        data: analytics.transaction_metrics
      };
    } catch (error) {
   ...
   * Get SDK-specific metrics
   */
  getSDKMetrics = async (query = {}) => {
    try {
      const analytics = await this.models.analytics.admin.calculateAnalytics(this.models, query);
      if (!analytics) {
        return {
          status: false,
          message: "Unable to calculate SDK metrics"
        };
      }
      return {
        status: true,
        data: analytics.sdk_metrics
      };
    } catch (error) {
      console.error("Error fetching SDK metrics:", error);
      return {
        status: false,
        message: "Failed to retrieve SDK metrics"
      };
    }
  };
  /**
   * Refresh analytics (force recalculation)
   */
  refreshAnalytics = async (query = {}) => {
    try {
      const analytics = await this.models.analytics.admin.calculateAnalytics(this.models, query);
      if (!analytics) {
        return {
          status: false,
          message: "Unable to refresh analytics"
        };
      }
      return {
        status: true,
...
var analytics_service_default = AdminAnalyticsService;

// src/features/app/admin/analytics/analytics.routes.ts
var createAnalyticsRouter = (data) => {
  const router2 = Router13();
  const service = new analytics_service_default(data.model);
  const controller = new analytics_controller_default(service);
  router2.get("/", controller.getAnalytics);
  router2.get("/latest", controller.getLatestAnalytics);
  router2.get("/date-range", controller.getDateRangeAnalytics);
  router2.get("/dashboard", controller.getDashboard);
  router2.get("/today", controller.getTodayAnalytics);
  router2.get("/weekly", controller.getWeeklyAnalytics);
  router2.get("/monthly", controller.getMonthlyAnalytics);
  router2.get("/merchants", controller.getMerchantMetrics);
  router2.get("/transactions", controller.getTransactionMetrics);
  router2.get("/sdk", controller.getSDKMetrics);
  router2.post("/refresh", controller.refreshAnalytics);
  return router2;
};

// src/features/app/admin/admin.routes.ts
var adminRouter = (data) => {
  const router2 = Router14();
  router2.use("/auth", auth_routes_default2(data));
  router2.use("/analytics", createAnalyticsRouter(data));
  return router2;
};
var admin_routes_default = adminRouter;

// src/shared/adapter/middleware/limiter/index.ts
import rateLimit2 from "express-rate-limit";

// src/shared/adapter/middleware/limiter/ipBlocklist.ts
var TEMP_BLOCK_DURATION = 24 * 60 * 60;
var MAX_SUSPICIOUS_ATTEMPTS = 3;
async function ensureHashStructure(key) {
  try {
    const type = await redis_default.type(key);
...
async function addToBlocklist(ip, reason, isPermanent = false, url) {
  try {
...
async function incrementSuspiciousAttempts(ip, url) {
  try {
    ...
    if (attempts >= MAX_SUSPICIOUS_ATTEMPTS) {
      ...
      await redis_default.hdel(REDIS_KEYS.SUSPICIOUS_ATTEMPTS, ip);
...
// src/shared/adapter/middleware/limiter/rateLimiter.ts
import rateLimit from "express-rate-limit";
import RedisStore from "rate-limit-redis";

// src/shared/adapter/middleware/limiter/ipBlocker.ts
function getIpAddress(req) {
  const forwarderIp = Array.isArray(req.headers["x-forwarded-for"]) ? req.headers["x-forwarded-for"][0] : req.headers["x-forwarded-for"]?.split(",").shift()?.trim();
...
// src/shared/adapter/middleware/limiter/constant.ts
loadConfig();
var config2 = {
  rateLimit: {
    api: {
...
function createRedisStore() {
  return new RedisStore({
    ...
    sendCommand: (...args) => redis_default.call(...args)
  });
}
var apiLimiter = rateLimit({
  windowMs: config2.rateLimit.api.windowMs,
  max: config2.rateLimit.api.max,
  ...
  validate: {
    ...
    singleCount: config2.environment !== "development"
  }
});
var loginLimiter = rateLimit({
  windowMs: config2.rateLimit.login.windowMs,
  max: config2.rateLimit.login.max,
...
var BURST_THRESHOLD = 30;
async function trackRequestBurst(ip) {
  ...
  try {
    ...
    if (now - tracker.timestamp > 1e3) {
      tracker.count = 1;
      tracker.timestamp = now;
    } else {
      tracker.count++;
    }
    await redis_default.setex(key, 60, JSON.stringify(tracker));
    return tracker;
  } catch (error) {
...
var ddosProtection = rateLimit2({
  store: createRedisStore(),
  windowMs: WINDOW_MS,
  max: MAX_REQUESTS,
  skip: (req) => ["/api/status", "/health-check"].includes(req.path),
...
async function burstDetection(req, res, next) {
  const ip = getIpAddress(req);
  try {
...
var isAdminRequest = (req, res, next) => {
  const ip = getIpAddress(req);
...
var adminRateLimit = rateLimit2({
  ...
  handler: async (req, res) => {
    ...
    try {
      await incrementSuspiciousAttempts(ip);
      const attempts = await redis_default.get(`${REDIS_KEYS.SUSPICIOUS_ATTEMPTS}:${ip}`);
...
var router = (data) => {
  const router2 = {
    ...
    bot: TelegrafRouter(data.repo.encryption)
  };
  const _adminRouter = admin_routes_default(data);
  const _merchantRouter = merchant_routes_default(data);
  const _sdkRouter = route_default(data);
  router2.router.use(getApiRequests());
  router2.router.use("/api/admin", adminRateLimit, isAdminRequest, _adminRouter);
  router2.router.use("/api/merchant", _merchantRouter);
  router2.router.use("/sdk-api", _sdkRouter.router);
  router2.socket.extendsRoutes(_sdkRouter.socket);
  router2.bot.extendsRoutes(bot_route_default(data));
  return router2;
};
...
var TransactionQueue = class {
  _transactionQueue;
  constructor() {
    this._transactionQueue = new Queue2("transaction-processing", {
      connection: redis_default,
      defaultJobOptions: {
  ...
  createWorker(callback) {
    return new Worker2("transaction-processing", callback, {
      connection: redis_default
    });
  }
};
...
}).m(function(Command2, cs, config3, o) {
  return [
...
import { Between as Between3, LessThanOrEqual, MoreThanOrEqual, Repository } from "typeorm";
...
var AdminAnalyticsRepository = class extends Repository {
  ...
  async calculateTransactionMetrics(models, periodStart, periodEnd) {
    ...
    const totalTransactions = await models.transactions.count({
      where: { created_at: Between3(periodStart, periodEnd) }
    });
    const successfulTransactions = await models.transactions.count({
      where: {
        created_at: Between3(periodStart, periodEnd),
        status: "success"
      }
    });
...
import { Between as Between4 } from "typeorm";
    ...
    const [current, previous] = await Promise.all([
      this.findOne({
        where: {
          ...
          type,
          created_at: Between4(startCurrent, endCurrent)
        }
      }),
      this.findOne({
        where: {
          account: { id: tx.account.account_id },
          type,
          created_at: Between4(startPrev, endPrev)
        }
      })
    ]);
...
var runApp = async () => {
  ...
  const router2 = routes_default({ model, repo, tools });
  ...
  router2.socket.runServer(httpServer);
  ...
  httpServer.listen(PORT, () => {
    ...
    if (process.env.ENV !== "production") {
      ...
      console.log(router2.socket.socketInfo.map((sock) => ({ path: sock.path, action: sock.action.name })));
    }
  });
  return { app, server: httpServer };
};
var index_default = runApp();
export {
